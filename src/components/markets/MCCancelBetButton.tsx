...
import { cancelBetMC } from '@/lib/markets/actions';
import { mcPreviewSell } from '@/lib/amm/cpmm';
import { toast } from 'sonner';

type MCCancelBetButtonProps = {
  positionId: string;
  optionIndex: number;
  shares: number;
  cost: number;
...
export function MCCancelBetButton({
  positionId,
  optionIndex,
  shares,
  cost,
  pools,
}: MCCancelBetButtonProps) {
  ...
  const preview = useMemo(() => {
    try {
      const decPools = pools.map((p) => new Decimal(p));
      const result = mcPreviewSell(
        { pools: decPools },
        optionIndex,
        new Decimal(shares)
      );
  ...
  }, [optionIndex, shares, cost, pools]);

  async function handleCancel() {
    setLoading(true);
    const result = await cancelBetMC({ positionId });
    setLoading(false);
