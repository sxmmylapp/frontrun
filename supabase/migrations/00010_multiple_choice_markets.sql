...
-- Phase 10: Support markets with 2-10 named outcomes
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Add market_type to markets
-- -----------------------------------------------------------------------------
ALTER TABLE markets ADD COLUMN market_type TEXT NOT NULL DEFAULT 'binary'
  CHECK (market_type IN ('binary', 'multiple_choice'));

-- -----------------------------------------------------------------------------
-- Relax resolved_outcome constraint to allow outcome labels (e.g., "Trump")
-- Binary: 'yes'/'no' or NULL. Multiple choice: any non-empty string or NULL.
-- Enforcement handled in RPCs.
-- -----------------------------------------------------------------------------
ALTER TABLE markets DROP CONSTRAINT IF EXISTS markets_resolved_outcome_check;

-- -----------------------------------------------------------------------------
-- market_outcomes: named outcomes for multiple choice markets
-- -----------------------------------------------------------------------------
...
CREATE POLICY "Authenticated users can read market outcomes"
  ON market_outcomes FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Service role can manage market outcomes"
  ON market_outcomes FOR ALL
  USING (true)
  WITH CHECK (true);

-- -----------------------------------------------------------------------------
-- outcome_pools: CPMM pool per outcome for multiple choice markets
-- -----------------------------------------------------------------------------
CREATE TABLE outcome_pools (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  market_id   UUID NOT NULL REFERENCES markets(id) ON DELETE CASCADE,
...
CREATE INDEX idx_outcome_pools_market ON outcome_pools(market_id);

ALTER TABLE outcome_pools ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read outcome pools"
  ON outcome_pools FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Service role can manage outcome pools"
  ON outcome_pools FOR ALL
  USING (true)
  WITH CHECK (true);

-- Enable realtime for outcome_pools
ALTER PUBLICATION supabase_realtime ADD TABLE outcome_pools;

-- -----------------------------------------------------------------------------
-- Extend positions: add outcome_id for multi-choice (NULL for binary)
-- Relax outcome CHECK to allow any non-empty string
-- -----------------------------------------------------------------------------
ALTER TABLE positions ADD COLUMN outcome_id UUID REFERENCES market_outcomes(id);
ALTER TABLE positions DROP CONSTRAINT IF EXISTS positions_outcome_check;

CREATE INDEX idx_positions_outcome_id ON positions(outcome_id);

-- -----------------------------------------------------------------------------
-- place_bet_mc: Atomic bet placement for multi-choice markets
-- N-way CPMM: product(pools) = k
-- Buy shares of outcome i: add tokens/(N-1) to each other pool,
-- then compute new pool_i = k / product(other new pools)
-- shares = pool_i_old - pool_i_new
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Compute k = product of all pools
  SELECT EXP(SUM(LN(pool))) INTO v_k
    FROM outcome_pools WHERE market_id = p_market_id FOR UPDATE;
...
-- -----------------------------------------------------------------------------
-- cancel_bet_mc: Sell shares back for multi-choice markets
-- Reverse N-way CPMM: shares return to target pool, scale other pools down
-- to maintain invariant k, extract the difference as tokens.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Compute k = product of all pools (lock all pool rows)
  SELECT EXP(SUM(LN(pool))) INTO v_k
    FROM outcome_pools WHERE market_id = v_position.market_id FOR UPDATE;
...
-- -----------------------------------------------------------------------------
-- resolve_market_mc: Resolve a multi-choice market
-- Winners = holders of the winning outcome. Total pool = sum of all pools.
-- Payout per share = total_pool / total_winning_shares.
-- -----------------------------------------------------------------------------
